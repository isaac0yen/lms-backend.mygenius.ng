...
const setJWT = async (id) => {
    try {
        ...
        const accessToken = jwt.sign({ userObject }, process.env.ACCESS_TOKEN_SECRET, { expiresIn: '7d' });
        if (!accessToken) {
        ...
        const refreshToken = await jwt.sign(refreshObject, process.env.REFRESH_TOKEN_SECRET, { expiresIn: '30d' });
        if (!refreshToken) {
