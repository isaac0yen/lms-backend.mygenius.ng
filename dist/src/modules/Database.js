...
const DBConnect = async () => {
    try {
        _DB = await (0, promise_1.createPool)({
        ...
        connection.release();
        console.log('New MySQL Connection pool created successfully. ', process.env.DB_DATABASE);
    }
    catch (error) {
...
exports.db = {
    ...
    async query(sql) {
        ...
        try {
            const [results] = await _DB.query(sql);
            return results;
        }
        catch (err) {
    ...
    async insertMany(tableName, data) {
        ...
        const [result] = await _DB.query(query, [
            values,
        ]);
    ...
    async findDirect(query, condition) {
        const [rows] = await _DB.execute(query, condition);
        return rows;
    },
